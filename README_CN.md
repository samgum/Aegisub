...
现代视频源兼容、中日韩文本工具、音频稳定性，以及批量时间轴修复。
...
- **打开 HDR/BT.2020 视频**——原版 Aegisub 会以"Unknown video color space"拒绝
  4K HDR 片源。本分支识别 BT.2020 及现代色彩空间标识，使 UHD/HDR/杜比视界
  文件可以打开。
- **预览级 HDR 色调映射**——标准 HDR10（PQ）和 HLG 基础层使用 16-bit RGB CPU
  路径，执行基于亮度的色调映射、BT.2020 到 BT.709 转换、色域压缩和输出空间抖动。
  该功能用于字幕预览，不是参考级调色渲染。
- **杜比视界限制**——不应用 RPU 重塑。有标准 PQ/HLG 基础层的配置只能预览该基础
  层；profile 5 等 IPT/ICtCp 源不受支持，其预览颜色不可靠。
- **预览降采样**——宽度超过 1920 的 HDR 源会在编解码器完成解码后转换成 1920
  宽预览，以降低色彩转换、上传和帧缓存开销，同时保持原始宽高比。
